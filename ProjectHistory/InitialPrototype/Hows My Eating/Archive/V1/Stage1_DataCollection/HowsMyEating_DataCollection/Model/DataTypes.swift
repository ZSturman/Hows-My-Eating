...
final class CapturedMotionAndMovieData {
    ...
    /// The monotonic reference time (e.g., CACurrentMediaTime) at which both video and motion recording started.
    var recordingReferenceTime: Double
    
    init(timestamp: Date, motionArray: [MotionData], moviePath: String, recordingReferenceTime: Double) {
        self.timestamp = timestamp
        self.motionArray = motionArray
        self.moviePath = moviePath
        self.recordingReferenceTime = recordingReferenceTime
    }
}
...
struct MotionData: Codable {
    ...
    /// Elapsed time in seconds since the recording reference time.
    ...
    init(timestamp: Double, relativeTimestamp: Double, attitude: Attitude, rotationRate: RotationRate, userAcceleration: UserAcceleration, gravity: Gravity, transformedRotation: TransformedRotation) {
        self.timestamp = timestamp
...
var defaultMotionData = MotionData(timestamp: 0.0, relativeTimestamp: 0.0, attitude: .zero, rotationRate: .zero, userAcceleration: .zero, gravity: .zero, transformedRotation: .zero)
