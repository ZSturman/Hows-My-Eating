...
final class MotionManager: NSObject, ObservableObject, CMHeadphoneMotionManagerDelegate {
    ...
    /// The monotonic reference time used to compute relative timestamps for synchronization with video recording start.
    /// This is set when recording starts and used to calculate elapsed time for each motion sample relative to this reference.
    private var referenceTime: Double? = nil
    ...
    private func collectMotionData(from motion: CMDeviceMotion) {
        ...
        let relTime = referenceTime != nil ? (now - referenceTime!) : 0.0
    ...
    /// Starts recording motion data from the provided monotonic reference time.
    /// The referenceTime is used to calculate relative timestamps of motion samples synchronized with video recording start.
    func startRecording(referenceTime: Double) {
        reset()
